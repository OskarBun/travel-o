const db = require('./db');
const request = require('async-request');
const generateName = require('sillyname');
const util = require('util')

let github = {
    client_id: process.env.GITHUB_ID,
    client_secret: process.env.GITHUB_SECRET
}

// dont use oAuth in local development
// requires return url which isnt available locally
if(!github.client_secret && !process.env.NODE_ENV === 'development'){
    github = require('../oauth.json').github;
}

// random colour is assigned to users unless changed
function randomColor(){
    const colors = [
        '#4D4D4D', '#999999', '#FFFFFF', '#F44E3B', '#FE9200', '#FCDC00',
        '#DBDF00', '#A4DD00', '#68CCCA', '#73D8FF', '#AEA1FF', '#FDA1FF',
        '#333333', '#808080', '#CCCCCC', '#D33115', '#E27300', '#FCC400',
        '#B0BC00', '#68BC00', '#16A5A5', '#009CE0', '#7B64FF', '#FA28FF',
        '#000000', '#666666', '#B3B3B3', '#9F0500', '#C45100', '#FB9E00',
        '#808900', '#194D33', '#0C797D', '#0062B1', '#653294', '#AB149E'
    ];

    return colors[Math.floor(Math.random()*colors.length)];
}

// RPC Methods
const methods = {
    async echo({message}, ctx){
        return [message, 'echo'];
    },

    async signOut(_, ctx){
        delete ctx.websocket.user;
        return ["done", null];
    },

    async _auth({cookie}, ctx){
        const user = await db.User.findById(cookie);
        if(user){
            const topic = `user/${user.id}`;
            ctx.broadcast.register(topic, ctx.websocket);
        }
        return user;
    },

    async deregister({topic}, ctx){
        if(topic instanceof Array){
            for(t of topic){
                ctx.broadcast.deregister(ctx.websocket, t)
            }
        } else {
            ctx.broadcast.deregister(ctx.websocket, topic)
        }
        return ["done", null]
    },

    async github({state}, ctx){
        //match the state with a auth callback, make the post, return user
        const code = await ctx.oauth.github.get(state)
        var options = {
            method: 'POST',
            headers: {
                Accept: 'application/json'
            },
            data: {
                client_id: github.client_id,
                client_secret: github.client_secret,
                code: code
            }
        };
        const github_auth = await request('https://github.com/login/oauth/access_token', options)
        const github_user = await request('https://api.github.com/user?access_token='+JSON.parse(github_auth.body).access_token, {
            headers: {
                "User-Agent": "travel-o"
            }
        });
        const gh_user = JSON.parse(github_user.body);
        let user = await db.User.findAll({
            where: {
                github_id: gh_user.id
            }
        });
        if(user.length === 0){
            user = await db.User.create({
                username: gh_user.login,
                avatar_url: gh_user.avatar_url,
                color: randomColor(),
                github_id: gh_user.id
            });
        } else {
            user = user[0];
        }

        ctx.websocket.user = user;

        const topic = `user/${user.id}`;
        ctx.broadcast.register(topic, ctx.websocket);
        return [user, topic];

    },


    async newUser(_, ctx){
        const username = generateName();

        const user = await db.User.create({
            username: username,
            color: randomColor()
        });

        ctx.websocket.user = user;

        const topic = `user/${user.id}`;
        ctx.broadcast.register(topic, ctx.websocket);
        return [user, topic];
    },

    async updateUser({color}, ctx){
        const user = ctx.websocket.user
        user.color = color;
        await user.save();
        const topic = `user/${user.id}`;
        return [user, topic];
    },

    //This needs register with filter
    async searchUser({query}, ctx){
        const users = await db.User.findAll({
            where: {
                username: {
                    $like: `%${query}%`
                }
            }
        });
        return [users, null];
    },

    async newTrip({title, access}, ctx){
        const user = ctx.websocket.user;

        // create trip
        const trip = await db.Trip.create({
            title: title,
            access: 'private'
        });

        //This could be done in the one above
        await trip.addPeople(user)

        //This is a bit hacky
        const _trip = trip.dataValues

        _trip.people = [user];

        const topic = `trip/${_trip.id}`;
        ctx.broadcast.register(topic, ctx.websocket);
        return [_trip, topic];
    },

    async getTrip({id, access}, ctx){
        const user = ctx.websocket.user;

        // create trip
        const trips = await user.getTrips({
            where: { id: id },
            include: [
                {
                    model: db.Destination,
                },
                {
                    association: db.Trip.People
                }
            ],
            order: [
                [db.Destination, 'order', 'ASC']
            ]
        });
        const trip = trips[0];

        const topic = `trip/${trip.id}`;
        ctx.broadcast.register(topic, ctx.websocket);
        trip.people.forEach(user=>{
            ctx.broadcast.register('user/'+user.id, ctx.websocket);
        })
        return [trip, null];
    },

    async updateTrip({title, trip_id}, ctx){
        const user = ctx.websocket.user;

        // create trip
        const trips = await user.getTrips({
            where: { id: trip_id },
            include: [
                {
                    model: db.Destination
                },
                {
                    association: db.Trip.People
                }
            ]
        });
        const trip = trips[0];

        trip.title = title;
        await trip.save();

        const topic = `trip/${trip.id}`;
        return [trip, topic];
    },

    async addDestination({trip_id, lat, lng, name, order}, ctx){
        const user = ctx.websocket.user;

        // find trip
        const destination = await db.Destination.create({
            lat,
            lng,
            name,
            trip_id,
            order,
            created_by: user.id
        });

        const topic = `trip/${trip_id}/destination/${destination.id}`
        return [destination, topic]
    },

    async addUser({trip_id, user_id}, ctx){
        const user = await db.User.findById(user_id)
        const trip = await db.Trip.findById(trip_id)

        await trip.addPeople(user)

        ctx.broadcast.register('user/'+user.id, ctx.websocket);
        const topic = `trip/${trip_id}/user/${user_id}`
        return [user, topic]
    },

    async orderDestinations({trip_id, destinations}, ctx){
        const trip = await db.Trip.findById(trip_id)
        for(i in destinations){
            await db.Destination.update(
                { order: i },
                { where: { id: destinations[i].id }})
        }
        const _destinations = await trip.getDestinations()

        const topic = `trip/${trip_id}/destination`
        return [_destinations, topic]
    },

    //This needs register with filter
    async searchTrips({query}, ctx){
        const user = ctx.websocket.user;

        // filter if query was set
        let trips = [];
        if(query) {
            trips = await user.getTrips({
                where: {
                    title: { $like: `%${query}%` }
                }
            });
        }
        else {
            trips = await user.getTrips();
        }
        const topic = `trip`;
        // ctx.broadcast.register(topic, (response)=>{
        //     if(response.result instanceof Array){
        //
        //     }
        // });
        return [trips, null];
    }
}

module.exports = methods;
